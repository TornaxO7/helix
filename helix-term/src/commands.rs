...
use helix_core::{
    ...
    Rope, RopeGraphemes, RopeReader, RopeSlice, Selection, SmallVec, Tendril, Transaction,
};
use helix_view::{
...
pub fn scroll_horizontal(cx: &mut Context, _offset: usize, direction: Direction) {
    let (view, doc) = current!(cx.editor);
    let text = doc.text();
    ...
    let cursor_pos = {
        ...
        let vertical_viewport_end = view.offset.vertical_offset + viewport.height as usize;
        ...
        .unwrap_or(visual_offset_from_block(
            text.slice(..),
            view.offset.anchor,
            cursor,
            &text_fmt,
